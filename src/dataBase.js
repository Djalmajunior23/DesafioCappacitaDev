const sequence = {

    _id: 1,

    get id() { return this._id++; }
}

const pokemons = []

function salvarPokemons(pokemon) {
    if (!pokemon.id) pokemon.id = sequence.id
    pokemons[pokemon.id] = pokemon
    return pokemon
}

function mostrarPokemon(id) {
    return pokemons[id] || {}
}

function mostrarPokemons() {
    return Object.values(pokemons)
}

function atualizarPokemon(id, pokemon) {
    pokemons[i] = pokemon
    return pokemon
}

function deletarPokemon(id) {
    seguence_id = sequence.id - 1
    pokemons.splice(id, 1)
    res.send(dataBase.deletarPokemon(req.params.id))
    pokemon.forEach(pokemon => {
        if (pokemon.id > id) {
            pokemon.id = pokemon.id - 1
        }
    })
    return pokemonDeletado
}

function batalhaPokemon(id1, id2) {
    const superEftivo = 40
    const efetivo = 20
    const naoEfetivo = 10

    const pokemon1 = pokemons[id1]
    const pokemon2 = pokemons[id2]

    if (pokemon1.hp != 0 && pokemon2.hp != 0) {
        if (pokemon1.tipo == pokemon2.fraqueza) {
            pokemon2.hp = pokemon2.h - superEfetivo
        } else if (pokemon1.tipo == pokemon2.resistencia) {
            pokemon2.hp = pokemon2.hp - naoEfetivo
        } else {
            pokemon2.hp = pokemmon2.hp - efetivo
        }
    }

    if (pokemon1.hp != 0 && pokemon2.hp != 0) {
        if (pokemon2.tipo == pokemon1.fraqueza) {
            pokemon1.hp = pokemon1.h - superEfetivo
        } else if (pokemon2.tipo == pokemon1.resistencia) {
            pokemon1.hp = pokemon1.hp - naoEfetivo
        } else {
            pokemon1.hp = pokemmon1.hp - efetivo
        }
    }
    if (pokemon1.hp < 0) pokemon1.hp = 0
    if (pokemon1.hp < 0) pokemon1.hp = 0

    return `${pokemon1.nome}: ${pokemon1.hp} /${pokemon2.nome}:{pokemon2.hp}`

}

function curarPokemon(id) {

    const pocaoHP = 20
    const recPokemon = pokemons[id]

    if (recPokemon.hp < 100) {
        recPokemon.hp += pocaoHP
        return `${recPokemon.nome}: ${recPokemon.hp}`

    } else if (recPokemon.hp = 100) {
        return `hp de ${recPokemon.nome} Pokemon já está com o hp máximo!`
    }

}

module.exports = { salvarPokemons, mostrarPokemon, mostrarPokemons, atualizarPokemon, deletarPokemon, batalhaPokemon, curarPokemon }